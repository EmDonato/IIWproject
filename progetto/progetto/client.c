...
int main(int argc, char *argv[ ]) {
  ...
  n = recvfrom(sockfd, (void *)(&pacchetto),sizeof(packet), 0 , NULL, NULL);
  if (n < 0) {
    perror("errore in recvfrom");
    exit(1);
  }
  if (n > 0) {
    pacchetto.data[n] = 0;        /* aggiunge il carattere di terminazione */
    printf("seqnumb: %d\nacknumb: %d\nheadlen: %d\nack: %d\nrst: %d\nsyn: %d\nfin: %d\n",pacchetto.seqnumb, pacchetto.acknumb, pacchetto.flags.headlen, pacchetto.flags.ack, pacchetto.flags.rst, pacchetto.flags.syn, pacchetto.flags.fin);
    if (fputs( pacchetto.data, stdout) == EOF)   {  /* stampa recvline sullo stdout */
